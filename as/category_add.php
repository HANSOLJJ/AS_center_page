...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($s5_category)) {
        ...
        // 자동으로 s5_caid 생성 (최대값 + 1)
        $max_id_result = mysql_query("SELECT MAX(CAST(s5_caid AS UNSIGNED)) as max_id FROM step5_category");
        $max_id_row = mysql_fetch_assoc($max_id_result);
        $max_id = $max_id_row['max_id'] ?: 0;
        $new_caid = str_pad($max_id + 1, 4, '0', STR_PAD_LEFT);

        // INSERT 쿼리
        $query = "
            INSERT INTO step5_category 
            (s5_caid, s5_category)
            VALUES 
            ('" . mysql_real_escape_string($new_caid) . "', '" . mysql_real_escape_string($s5_category) . "')
        ";

        if (mysql_query($query)) {
            header('Location: parts.php?tab=tab2&inserted=1');
            exit;
        } else {
            $error_message = '카테고리 추가에 실패했습니다: ' . mysql_error();
        }
    }
}
?>
        ...
        <div class="content">
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="s5_category" name="s5_category" placeholder="카테고리명을 입력하세요" required>
                </div>
